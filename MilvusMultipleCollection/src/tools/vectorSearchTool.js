...
const { milvusClient } = require('../../src/config/milvusClient'); // Import our Milvus client
...
class VectorSearchTool extends Tool {
    ...
    description = `
    Performs vector similarity search (semantic search) or hybrid search in a specific Milvus collection.
    ...
    {
        "collection_name": string, // The name of the Milvus collection to search in. Mandatory.
    ...
    schema = z.object({
        collection_name: z.string().describe("The name of the Milvus collection to search in."), // Added collection_name
    ...
    async _call(input) {
        ...
        if (!collection_name) {
            const errorMessage = "Error: 'collection_name' is required for vector_search_tool.";
            console.error(`[VectorSearchTool] ${errorMessage}`);
            return JSON.stringify({ error: errorMessage });
        }

        try {
            console.log(`[VectorSearchTool] Querying collection '${collection_name}'... (assuming it is already loaded)`);
            ...
            console.error(`[VectorSearchTool] Error during Milvus vector search for collection '${collection_name}':`, error);
            return `Error performing Milvus vector search for collection '${collection_name}': ${error.message}. Please check the filter syntax or Milvus connection.`;
        } finally {
            console.log(`[VectorSearchTool] Finished operation on '${collection_name}'. (Collection release handled externally)`);
        }
    }
}
