...
const cohere = new CohereClient({
  token: process.env.CO_API_KEY  // 🔑 Replace with your real API key
});
...
const initializeEmbeddings = async ({ deleteExisting = false } = {}) => {
  try {
    if (deleteExisting) {
      console.log("🧹 Deleting collection completely to reset dimension...");
...
if (!generations || !Array.isArray(generations) || !generations[0]?.text) {
  ...
  try {
    ...
    const prompt = `
    You are a helpful and friendly assistant. Your job is to answer questions using the provided context.
