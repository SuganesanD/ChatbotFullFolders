...
async function embed_fetchedData_gemini(textChunks,empInfo) {
    ...
    const collection = await chroma.getCollection({ name: 'employee-embeddings2' });
...
const initializeEmbeddings = async ({ deleteExisting = false } = {}) => {
    try {
        if (deleteExisting) {
            ...
            const collectionExists = collections.includes('employee-embeddings2');
        
            if (collectionExists) {
                console.log("🧹 Deleting existing collection to reset dimension...");
                await chroma.deleteCollection({ name: 'employee-embeddings2' });
            ...
            await chroma.createCollection({ name: 'employee-embeddings2' }); 
...
async function classifyPromptGemini(query) {
    ...
    const nResults = isBroadQuery ? 5 : 10;
    console.log("nResults:", nResults);
    const end = Date.now();
