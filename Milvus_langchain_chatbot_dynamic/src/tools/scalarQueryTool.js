...
class ScalarQueryTool extends Tool {
    ...
    Input should be a JSON string with the following properties:
    {
        ...
        "collectionName: string,  // A milvus collection name in which this filter is going to be applied.                         
    }
    ...
    schema = z.object({
        ...
        collectionName:z.string().describe("A milvus collection name from which the filter is going to be applied.")
    });

    async _call(input) {
        const { filter, outputFields ,collectionName} = input;
