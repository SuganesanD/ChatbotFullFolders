...
const askHandler = async (req, res) => {
  try {
    ...
    const isValid = validateSchema(classified);
    if (!isValid) {
      logger.error(`❌ Invalid schema: ${JSON.stringify(classified)}`);
      return res.status(400).json({ error: 'Invalid classification structure' });
    }

    // 3. Route to appropriate logic function (fetch from vector db, filters, etc.)
    const { data, context } = await routeQuery(classified);
    logger.info(`📥 Retrieved Context: ${context?.length || data?.length || 0} items`);

    // 4. Generate clean final answer using LLM
    const answer = await generateAnswer(context, query, classified);
