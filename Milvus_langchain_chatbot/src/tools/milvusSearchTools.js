// src/tools/milvusSearchTools.js
...
class MilvusSearchTool extends Tool {
    ...
    schema = z.object({
        ...
        topK: z.number().optional().default(10).describe("The maximum number of results to return."),
    });
    ...
    async _call(input) {
        ...
        try {
            ...
            const searchParams = {
                ...
                limit: topK,                // Number of nearest neighbors to return (top-level)
                ...
                // Search parameters for the IVF_FLAT index
                search_params: {
                    anns_field: "embedding", // The vector field to search on
                    metric_type: "COSINE",   // Must match the index metric type
                    topk: topK,              // Add topk directly in search_params
                    params: JSON.stringify({ nprobe: 4 }) // Only nprobe in params
                },
            ...
            console.log("Milvus Search Tool: Full search parameters being sent:", JSON.stringify(searchParams, null, 2)); // Added logging for full params

            // Perform the search in Milvus
            const results = await milvusClient.search(searchParams);

            console.log("Milvus Search Tool: Raw Milvus results received:", JSON.stringify(results, null, 2)); // Added logging for raw results
            ...
            if (results && results.status && results.status.error_code === "Success" && results.results && results.results.length > 0) {
                const hits = results.results; // Directly use results.results
